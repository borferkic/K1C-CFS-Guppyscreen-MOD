#ifndef __POWER_SCREEN_H__
#define __POWER_SCREEN_H__
...
class PowerScreen {
 ...
 public:
  PowerScreen();
  PowerScreen(PowerScreen &o) = delete;
  void operator=(const PowerScreen &) = delete;

  std::mutex &get_lock();

  void connect_ws(const std::string &url);
  static PowerScreen *get();
  static PowerScreen *init(std::function<void(lv_color_t, lv_color_t)> hal_init);
  static void loop();
