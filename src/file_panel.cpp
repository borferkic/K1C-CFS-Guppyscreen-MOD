...
FilePanel::FilePanel(lv_obj_t *parent)
  ...
  , last_printed_value(lv_label_create(detail_cont))
{
  ...
  const char *titles[] = {"Print Time:", "Filament Weight:", "Last Printed:"};
  lv_obj_t *values[] = {print_time_value, filament_weight_value, last_printed_value};
...
void FilePanel::set_last_printed(const std::string &last_printed) {
  lv_label_set_text(last_printed_value,
                    last_printed.empty() ? "(unknown)" : last_printed.c_str());
}
